...
const RETINOL_BLOCK =
  "I don't hand retinol out. It's the fastest way I know to wreck a face that was fine last week. Get the boring part right for a month. Cleanser, moisturiser, SPF. Then ask me again and I'll tell you honestly whether you need it."

/** E-02.2: shade questions are answered with a question, never a guess. */
const SHADE_REPLY =
  "What are you wearing at the moment? Shade is the one thing I won't guess at. Tell me what you've got on and whether you like how it sits, and I'll tell you where you land."
...
/**
 * Her register, from E-02 and the shelf: name, price, full stop. The note is
 * its own sentence when it earns one. No dashes holding clauses together —
 * that is the tell of copy nobody spoke out loud, and generic automation is
 * the thing she says she cannot stand (sheet 03).
 */
const priced = (n: string) => {
  const p = byName(n)
  return p ? `${p.name}, ${gbp(p.price)}` : n
}
const withNote = (n: string) => {
  const p = byName(n)
  return p ? `${p.name}, ${gbp(p.price)}. ${p.note}` : n
}
...
function budgetLine(total: number, band: string, names: string[], text: string): string | null {
  ...
  const fallback = `start with ${priced(affordable)}${later ? `. Add the ${byName(later)?.name ?? later} next month. It works in that order` : ''}`
  return worthStretching
    ? `That's ${gbp(total)}, not ${gbp(ceiling)}. Stretch ${gbp(over)} if you can. It's the right pair. If you can't, ${fallback}.`
...
const skipLine = () => {
  ...
  return `And if the ${gbp(p.price)} one is on your list: good, not ${gbp(p.price)} good. I like it. I wouldn't pay that for it.`
}
...
function draftFor(text: string, a: JevAnswers): string | undefined {
  ...
  if (owned.length && MISFIT.test(text)) {
    ...
    const out = [
      p.hedged
        ? `You did nothing wrong. ${p.note} It's the one thing on that shelf I don't reach for, and I should say that louder.`
        : `Nothing wrong with you. It's ${/^8/.test(String(p.maya)) ? 'an' : 'a'} ${p.maya} from me and it still won't suit every face. That happens. It isn't something you did.`,
    ]
  ...
  if (intent === 'info' || SHADE.test(text)) {
    ...
    if (owned.length) return `${withNote(owned[0].name)} ${owned[0].maya} from me, and that's after using it rather than being sent it.`
    return undefined
  }
  ...
  switch (intent) {
    case 'diagnosis': {
      ...
      return `Easy to sort out. Tight after you wash it, dry. Shiny by lunch, oily. Both at once, combination. Red and cross most of the time is its own thing. Tell me which and I'll give you two products, not ten. ${INTAKE.using}`
    }
    case 'constraint': {
      ...
      if (ceiling !== null && ceiling !== undefined && total > ceiling) {
        if (first.price <= ceiling) {
          lines.push(`At ${gbp(ceiling)}, one product. ${withNote(first.name)} Keep the cleanser you've got and add ${cleanser.name} when you can.`)
        } else {
        ...
        lines.push(`Two, then. ${priced(cleanser.name)}. ${withNote(first.name)}`)
      }
      lines.push(`That's the ${POSTS.fiveMinute.title.replace(/^My /, '').toLowerCase()} I filmed. Least watched thing I've made. The one people actually stick to.`)
      break
    }
    case 'value': {
      ...
      // Only claim it tops its category when it actually does. She is trusted
      // because she is specific and honest (sheet 03); a flattering line that
      // is not true costs more than it buys.
      const best = SHELF.filter((x) => x.type === p.type).every((x) => x.maya <= p.maya)
      lines.push(
        best
          ? `Yes, and you're not being influenced. ${p.maya} from me, the highest I've given any ${p.type.toLowerCase()}. ${p.note}`
          : `Yes, worth it. ${p.maya} from me. ${p.note}`,
      )
    ...
    case 'context': {
      ...
      if (DRYNESS.test(text) || skinOf(a) === 'dry') {
        lines.push(`You've got ${have.name}, so texture's covered. A richer cream on top only earns its place if you're tight or flaking. If you are, ${priced('Cloud Cream')}.`)
      } else {
        lines.push(`You've got ${have.name}. ${have.note} That's the job done. You don't need a second one sitting on top of it.`)
      }
      break
    }
    case 'judgement': {
    ...
    case 'recommendation': {
      ...
      if (
        ...
        return `Not at ${gbp(ceiling)}. Not honestly. The cheapest thing I'd put on your skin is ${priced(cheapest)}, so keep the money until you've got that. Buying something worse in the meantime is how people end up with a shelf of things they never use.`
      }
      const opener =
        intent === 'transfer_of_trust'
          ...
          : intent === 'personalisation'
            ? `If I had your skin: `
            : ''
      lines.push(`${opener}${withNote(pick.names[0])}${pick.names[1] ? ` ${withNote(pick.names[1])}` : ''}`)
